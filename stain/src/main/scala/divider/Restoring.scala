...
object Restoring {
    ...
    def RestoringDividerUnsign(len: BigInt, input1: BigInt, input2: BigInt): (BigInt, BigInt) = {
      ...
      (while (j >= BigInt(1)) {
        ...
        //   R = R - input2 * pow2(2, j)
        Q = Q + Qbit * lshift
      }) invariant(j >= 0 && R < input2 * lshift && R >= 0 && R == input1 - input2 * Q && lshift == pow2(j)) // && && (((tmpR < tmpD) && (Qbit == 0)) || ((tmpR >= tmpD) && (Qbit == 1))) && (((Qbit == 1) && (R == tmpR - tmpD)) || ((Qbit == 0) && (R == tmpR)))) // && Q >= 0 && Q <= pow2(n) - pow2(j - 1)) 
