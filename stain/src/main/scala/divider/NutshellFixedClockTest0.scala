...
object NutshellFixedClockTest0 {
  case class RegVar(var state: String, var shiftReg: BigInt, var cnt: BigInt)
  ...
  def NutshellFixedClock(len: BigInt, input1: BigInt, input2: BigInt): Unit = {
    val RegVar = new RegVar("s_idle", 0, 0)
    while (true) {
      val (resR, resQ) = OneClock(RegVar, len, input1, input2)
    }
  }

  def OneClock(RegVar: RegVar, len: BigInt, input1: BigInt, input2: BigInt): (BigInt, BigInt) = {
    ...
    val (state_next, cnt_next, shiftReg_next) = RegVar.state match { 
      case "s_log2" => ("s_shift", BigInt(0), RegVar.shiftReg) 
      case "s_shift" => ("s_compute", RegVar.cnt, aValx2Reg)
      case "s_compute" if RegVar.cnt == len - 1 => ("s_finish", RegVar.cnt, RegVar.shiftReg)
      case "s_compute" if RegVar.cnt != len - 1 => ("s_compute", RegVar.cnt + 1, shiftReg_next_1) 
      case "s_finish" => ("s_idle", RegVar.cnt, RegVar.shiftReg)
      case _ => ("s_idle", RegVar.cnt, RegVar.shiftReg)
    }
