  ...
  l1 match {
    case Cons(h, t) => {
      h match {
        case true => {
          Sum(l) ==:| trivial |:
          Sum(true :: (t ++ l2)) ==:| trivial |:
          BigInt(1) + Sum(t ++ l2) ==:| lemma_Sum_take(t ++ l2, t, l2) |:
          BigInt(1) + Sum(t) + Sum(l2) ==:| trivial |:
          Sum(l1) + Sum(l2)
        }.qed
        case false => {
          Sum(l) ==:| trivial |:
          Sum(false :: (t ++ l2)) ==:| trivial |:
          BigInt(0) + Sum(t ++ l2) ==:| lemma_Sum_take(t ++ l2, t, l2) |:
          BigInt(0) + Sum(t) + Sum(l2) ==:| trivial |:
          Sum(l1) + Sum(l2)
        }.qed
      }
    }
...
def ToZ_matrix(l: List[List[Boolean]]): List[BigInt] = {
  ...
  val toZ_row = l match {
    case Cons(h, t) => toZ(h.map(Bool(_).asBigInt)) :: ToZ_matrix(t)
    case Nil() => Nil[BigInt]()
  }
  toZ_row
