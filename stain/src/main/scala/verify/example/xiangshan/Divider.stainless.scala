...
case class Divider(len: BigInt) {
  ...
  def regsRequire(regs: DividerRegs): Boolean = regs match {
    case DividerRegs(state, shiftReg, cnt, aSignReg, qSignReg, bReg, aValx2Reg) =>
      state.width == 3 &&
  ...
  def trans(inputs: DividerInputs, regs: DividerRegs): (DividerOutputs, DividerRegs) = {
    ...
    val newReq       = ((regs.state === s_idle) && (io_in_ready && io_in_valid))
