...
*/
  ...
  def Div(cfg: MulDivParams, width: BigInt, nXpr: BigInt = 32, inputs: MultiplierIO, regs: MulRegs, step: BigInt): (BigInt, MulRegs) = {  
    ...
    require(regs.count >= 0 && regs.count < width / cfg.divUnroll)
    require(regs.divisor == inputs.in2)
    require(step == BigInt(0) && regs.remainder == inputs.in1
      || step >= BigInt(1) && regs.remainder >= BigInt(0))
    decreases(width / cfg.divUnroll - regs.count)
    require(step == regs.count)
    require(regs.R == regs.remainder / Pow2(regs.count))
    require(regs.Q == regs.remainder - regs.R * Pow2(regs.count))
    require(regs.R + inputs.in2 * regs.Q * Pow2(width - regs.count + 1) == 2 * inputs.in1)
